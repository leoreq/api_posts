...
class TestAPI(unittest.TestCase):
    ...
    def test_delete_post(self):
        ...
        #targetpost=session.query(Post).get(postB.id)
        response2 = self.client.delete("/api/posts/{}".format(postB.id),headers=[("Accept", "application/json")])

        self.assertEqual(response2.status_code, 204)
        self.assertEqual(response2.mimetype, "application/json")

        self.assertEqual(response2.data.decode("ascii"), '')
    ...
    def test_get_posts_with_two_titles(self):
        """ Filtering posts by two title """
        postA = models.Post(title="Post with bells", body="Just a test")
        postB = models.Post(title="Post with whistles", body="Still a test")
        postC = models.Post(title="Post with bells and whistles",
                            body="Another test")
